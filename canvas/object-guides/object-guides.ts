...
import { PROPGUIDE } from "../constants";
...
function ObjectMoving(
   ...
   if (Math.abs(centerX - canvasMidx) <= snapSize) {
      if (!vm) {
         lines.push(new DefaultLine([canvasMidx, 0, canvasMidx, canvas.height], lineProp));
         vm = true;
      }
      e.target.set({
   ...
   if (Math.abs(centerY - canvasMidy) <= 10) {
      if (!hm) {
         lines.push(new DefaultLine([0, canvasMidy, canvas.width, canvasMidy], lineProp));
         hm = true;
      }
   ...
   canvas.getObjects().forEach((o) => {
      if (o.get("id") === e.target.get("id") || o.get("id") === PROPGUIDE) return;
      ...
      if (Math.abs(t - (top + height / 2)) <= snapSize) {
         if (!hm) {
            lines.push(new DefaultLine([l + oWidth * 0.5, t, left + width * 0.5, t], lineProp));
            hm = true;
         }
         e.target.set({
            top: t - height / 2,
      ...
      if (Math.abs(t + oHeight * 0.5 - (top + height * 0.5)) <= snapSize) {
         if (!hm) {
            lines.push(
               new DefaultLine(
                  [l + oWidth * 0.5, t + oHeight * 0.5, left + width * 0.5, t + oHeight * 0.5],
                  lineProp,
               ),
            );
            hm = true;
      ...
      if (Math.abs(l - (left + width * 0.5)) <= snapSize) {
         if (!vm) {
            lines.push(new DefaultLine([l, t + oHeight * 0.5, l, top + height * 0.5], lineProp));
            vm = true;
         }
      ...
      if (Math.abs(l + oWidth * 0.5 - (left + width * 0.5)) <= snapSize) {
         if (!vm) {
            lines.push(
               new DefaultLine(
                  [l + oWidth * 0.5, t + oHeight * 0.5, l + oWidth * 0.5, top + height * 0.5],
                  lineProp,
               ),
            );
            vm = true;
